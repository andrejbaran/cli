...
describe('account:signup', () => {
  ...
  baseTest
    ...
    .it('runs signup', async ctx => {
      ...
      expect(ctx.stdout).to.contain('💻 CTO.ai Ops - The CLI built for Teams 🚀')
      ...
      expect(ctx.stdout).to.contain('🎉 We just sent you an email with tips on how to get started!')
      expect(config.user.username).to.equal(username)
      expect(config.user.email).to.equal(email)
    })
  test
    .it('validates email to not allow invalid formats', async _ctx => {
  ...
  test
    .it('confirms password', async _ctx => {
